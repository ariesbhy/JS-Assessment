...
function isMenuItemInCategory(menuItem, category) {
  // Your code here
  let itemsMenu = category.name;
  let isMenuItemInCategory = menu.some((category) => {
...
function listMenuItemNamesByCategory(menu, category) {
  // Your code here
  let menuNames = menu.forEach((category) => {
    if (menu == category) {
      menu.push.category;
    }
  });
...
function getCheapestMenuItem(menu) {
  // Your code here
  let lowestPrices = menu.prices.filter((price) => {
    if (price < 0) {
      return lowestPrices;
    } else {
      false;
...
function getMenuItemsByIngredient(menu, ingredient) {
  // Your code here
  let names = menu.filter((ingredient) => {
    return names;
  });
}
...
function removeMenuItemById(menu, menuItemId) {
  // Your code here
  let menuItemId = menu.id;
  let filterdId = id.filter((menuId) => {
